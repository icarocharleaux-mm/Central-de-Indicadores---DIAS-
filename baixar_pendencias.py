...
# ── Credenciais ───────────────────────────────────────────────────────────────
USUARIO = os.environ.get("DIASLOG_USUARIO", "icaro.nascimento")
SENHA   = os.environ.get("DIASLOG_SENHA",   "98409672")
...
def consolidar(arquivos: list[Path]) -> Path:
    ...
    for arq in arquivos:
        try:
            df = pd.read_excel(arq, engine="openpyxl", header=3)
        except Exception:
            df = pd.read_excel(arq, engine="xlrd", header=3)
        df = df.dropna(how="all")  # remove linhas totalmente vazias
        log(f"  {arq.name}: {len(df)} linhas")
        frames.append(df)
